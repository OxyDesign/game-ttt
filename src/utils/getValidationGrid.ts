...
export const getValidationGrid = (grid: Grid): ValidationGrid => {
  ...
  const validGrid = Array.from(Array(lineLength)).map(() =>
    Array.from(Array(lineLength)).map(() => false),
  );
  ...
  for (let i = 0; i < lineLength; i++) {
    ...
    for (let j = 0; j < lineLength; j++) {
      ...
      if (j === lastIndex) {
        validGrid[i] = validGrid[i].map(() => true);
      }
    }
  }
